...
interface ExperienceDetail {
  ...
  guide: { id: string; full_name: string; bio: string | null; languages: string[] } | null;
}
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        .from("experiences")
        .select(
          "id, title, description, tags, price_usdc, duration_minutes, location, guide:guide_id ( id, full_name, bio, languages )"
        )
      ...
      <Card>
        ...
        <div className="mt-6 flex items-center justify-between rounded-lg bg-brand-bg p-4">
          <div>
            ...
            <p className="text-xs text-brand-muted">Optional - shown for your reference, doesn&apos;t block booking.</p>
          </div>
          <WalletConnectButton />
        </div>
