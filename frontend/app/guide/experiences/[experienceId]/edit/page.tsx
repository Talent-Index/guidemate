  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        .from("experiences")
        .select(
          "id, guide_id, title, description, tags, category, price_usdc, duration_minutes, location, image_url, image_urls, itinerary, is_active, status, wizard_step, meeting_lat, meeting_lng, meeting_label, slug, created_at, payout_destination"
        )
        .eq("id", experienceId)
        .eq("guide_id", guideId)
        .maybeSingle();
